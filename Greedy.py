import heapq
class node:
    def __init__(self, x, y, from_node, h):
    ...
    def __gt__(a, b): # operator > overloading for heap queue which is used by greedy algorithm
        if  a.h > b.h:
            return True
        else:
            return False
    def __lt__(a, b): # operator < overloading for heap queue which is used by greedy algorithm
        if a.h < b.h:
            return True
        else:
            return False
...
def make_child_node(current_node, parrent_node, height, width, maze):
    ...
    if not(x == parrent_node.x and y == parrent_node.y):
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            ...
            current_node.to.append(node(x,y,current_node, h)) # make child node
            heapq.heappush(hq, node(x,y,current_node, h)) # and push the child node to the heap queue
    ...
    if not(x == parrent_node.x and y == parrent_node.y):
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            h=width-x+height-y-2
            current_node.to.append(node(x,y,current_node, h))
            heapq.heappush(hq, node(x,y,current_node, h))
            check[y][x] = 1
...
for i in range(0, int(testcases)):
    ...
    for j in range(0, height):
        ...
        for k in range(0, width):
            maze[j][k] = int(line[k])
    start_node = node(0,0,None, width+height-2)
