...
def make_child_node(current_node, parrent_node):
    ...
    x = current_node.x
    y = current_node.y + 1
    if not(x == parrent_node.x and y == parrent_node.y) and goal_state[0] == False:
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            h=width-x+height-y-2
            child = node(x,y,current_node, current_node.g, h)
            current_node.to.append(child) # make child node
            check[y][x] = 1 # check array is for checking if the node of coodinates (y,x) has been visited
            make_child_node(child, current_node) # search from the child node
    ...
    if not(x == parrent_node.x and y == parrent_node.y) and goal_state[0] == False:
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            h=width-x+height-y-2
            child = node(x,y,current_node, current_node.g, h)
            current_node.to.append(child)
            check[y][x] = 1
            make_child_node(child, current_node)
...
for i in range(0, int(testcases)):
    ...
    make_child_node(start_node, start_node)
f.close()
f_out.close()
