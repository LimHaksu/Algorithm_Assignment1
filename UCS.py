import heapq
class node:
    def __init__(self, x, y, from_node, g):
    ...
    def __gt__(a, b): # operator > overloading for heap queue which is used by UCS algorithm
        if a.g > b.g:
            return True
        else:
            return False
    def __lt__(a, b): # operator < overloading for heap queue which is used by UCS algorithm
        if a.g < b.g:
            return True
        else:
            return False
...
def make_child_node(current_node, parrent_node, height, width, maze):
    ...
    if not(x == parrent_node.x and y == parrent_node.y):
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            current_node.to.append(node(x,y,current_node, current_node.g+1)) # make child node
            heapq.heappush(hq, node(x,y,current_node, current_node.g+1)) # and push the child node to the heap queue
    ...
    if not(x == parrent_node.x and y == parrent_node.y):
        if width > x >= 0 and height > y >= 0 and maze[y][x] == 0 and check[y][x] == 0:
            current_node.to.append(node(x,y,current_node, current_node.g+1))
            heapq.heappush(hq, node(x,y,current_node, current_node.g+1))
            check[y][x] = 1
